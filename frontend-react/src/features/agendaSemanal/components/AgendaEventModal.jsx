...
import { MenuPacientesModal as AgendaPatientSearchModal, mapAgendaPatientToDraft } from '../../menuPacientes/components/MenuPacientesModal.jsx';
import { listarMenuPacientes } from '../../menuPacientes/api/menuPacientesApi.js';
...
export function AgendaEventModal({ editor, onCancel, onNew, initialTab = 'dados', initialDuration = 5, statusCatalog = [], providerId = '', unitId = '', onCreated }) {
  ...
  const resolvePatientName = async (trigger = 'enter') => {
    ...
    try {
      const result = await listarMenuPacientes({ q: draft.name, offset: 0, limit: 20 });
      const exactMatch = (result?.items || []).find((patient) => String(patient.nome || patient.name || '').trim().toLocaleLowerCase() === draft.name.trim().toLocaleLowerCase());
  ...
  return <>
    <Modal open title={isNew ? 'Edita agendamento' : 'Editar agendamento'} className="agenda-event-modal" width={608} centered maskClosable={false} destroyOnClose footer={null} onCancel={onCancel} styles={{ body: { padding: 0 } }}>
        ...
        {patientSearchRequested ? <AgendaPatientSearchModal open initialQuery={draft.name} onCancel={closePatientSearch} onSelect={(patient) => { setDraft((current) => ({ ...current, ...mapAgendaPatientToDraft(patient), patientIdExplicit: true })); closePatientSearch(); }} /> : null}
