...
export function ConfiguracaoPreferenciasModal({ open, onClose, targetUser = null, origin = 'menu' }) {
  const [activeTab, setActiveTab] = useState('geral');
  const { values, options, user, loading, saving, error, update, save } = useConfiguracaoPreferenciasGeral(open, targetUser);
  const modelos = useConfiguracaoPreferenciasModelos(open, targetUser);
  const ambiente = useConfiguracaoPreferenciasAmbiente(open, targetUser);
  const dados = useConfiguracaoPreferenciasDadosUsuario(open, targetUser);
  const odontograma = useConfiguracaoPreferenciasOdontograma(open, targetUser);
  ...
  return <BranaModal open={open} onCancel={onClose} footer={null} width={690} centered destroyOnClose maskClosable={false} keyboard title={`Configura preferências do usuário (${titleName})`} className="config-preferencias-modal" data-origin={origin} styles={{ body: { padding: '8px 10px 10px' } }}>
