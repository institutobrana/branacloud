...
export function useConfiguracaoPreferenciasAmbiente(open, targetUser) {
  ...
  useEffect(() => {
    ...
    getEnvironmentPreferences(targetUserId).then((data) => { if (!cancelled) { setValues(normalizeEnvironment(data?.values)); setOptions(data?.options || {}); } })
  ...
  const save = useCallback(async () => {
    ...
    try { const data = await updateEnvironmentPreferences(values, targetUserId); setValues(normalizeEnvironment(data?.values)); setOptions(data?.options || options); return data; }
