(function(){
  ...
  function ensureUI(ctx={}){
    ...
    const pageDefault=ctx.editorTextosPageDefault||{};
    const makeDraggable=(modal,header)=>{
      if(modal instanceof HTMLElement&&header instanceof HTMLElement&&typeof ctx.ensureChromeDraggable==="function"){
        try{
          ctx.ensureChromeDraggable(modal,header);
    ...
    Object.assign(shell,{
      ...
      openSelId:null,
      openContextId:null,
      openDeleteResolve:null,
      pdfPromptResolve:null,
      modeloAtualId:null,
      modeloAtualArquivo:null,
      modeloAtualSistema:false,
      tipoAtual:"outros",
      extensaoAtual:".txt",
      formatoAtual:"text",
      alterado:false,
      ...
      paginaConfig:{...pageDefault},
      rulerDrag:null,
      rulerMetrics:null,
      tableResize:null,
      tableResizeBound:false,
      imageOverlay:null,
      imageSelectedEl:null,
      imageResize:null,
      imageResizeBound:false,
      imageDocBound:false,
      assistContext:null,
      assistPacienteId:0,
      assistMedicamentos:[],
      assistItensIncluidos:[],
      assistConteudoBase:null,
      assistConteudoBaseFormato:"text",
      assistConteudoBaseAlterado:false,
      assistPreviewBaseConteudo:null,
      assistPreviewBaseFormato:"text",
      assistPreviewAplicado:false,
      assistMedMenuSource:[],
      assistMedMenuFiltered:[],
      assistMedMenuSelId:null,
      assistMedMenuAlphaAtual:"*",
      assistAtestadoContext:null,
      assistAtestadoPacienteId:0,
      assistAtestadoCidMenuItems:[],
      assistAtestadoCidMenuSelId:null,
      assistAtestadoCidMenuAlphaAtual:"*",
      signBridgeHealth:null,
      signBridgeCerts:[],
      signBridgeSelectedThumbprint:"",
      imagePending:null,
      toolbarSyncScheduled:false,
      toolbarSelectionHandler:null,
      documentModelFeatureEnabled:true,
      documentModel:typeof ctx.editorTextosDocumentoModelCriar==="function"?ctx.editorTextosDocumentoModelCriar():null,
      documentModelSeq:0,
      documentModelUpdateTimer:null,
      documentModelUpdating:false,
      documentModelComposing:false,
      documentModelDirtyFlag:false,
      documentModelLastSignature:"",
      documentModelLastRenderedSignature:"",
      modelRenderSandbox:null,
      paragraphModels:new Map(),
      paragraphSeq:0,
      paragraphTabOpSeq:0
    });
    if(shell.openTipo){
      shell.openTipo.innerHTML=(Array.isArray(ctx.editorTextosOpenTipoOpcoes)?ctx.editorTextosOpenTipoOpcoes:[]).map(item=>`<option value="${escHtml(String(item.value))}">${escHtml(String(item.label))}</option>`).join("");
      shell.openTipo.value="rich";
    }
    makeDraggable(shell.signBackdrop?.querySelector?.(".editor-textos-sign-modal"),shell.signBackdrop?.querySelector?.(".modal-header"));
    makeDraggable(shell.assistBackdrop?.querySelector?.(".editor-textos-assist-modal"),shell.assistBackdrop?.querySelector?.(".modal-header"));
    makeDraggable(shell.assistMedMenuBackdrop?.querySelector?.(".editor-textos-assist-medmenu-modal"),shell.assistMedMenuBackdrop?.querySelector?.(".modal-header"));
    makeDraggable(shell.assistAtestadoBackdrop?.querySelector?.(".editor-textos-atestado-modal"),shell.assistAtestadoBackdrop?.querySelector?.(".modal-header"));
    if(shell.mergeBackdrop){
      const mergeModal=shell.mergeBackdrop.querySelector?.(".editor-textos-merge-modal");
      const mergeHeader=mergeModal?.querySelector?.(".modal-header");
      makeDraggable(mergeModal,mergeHeader);
    }
    if(shell.openBackdrop){
      const openModal=shell.openBackdrop.querySelector?.(".editor-textos-open-modal");
      const openHeader=openModal?.querySelector?.(".modal-header");
      makeDraggable(openModal,openHeader);
    }
    return shell;
  }
  ...
  function toolbarAtualizarVisualComboCor(cfg){
    if(!(cfg?.color instanceof HTMLSelectElement))return false;
    const select=cfg.color;
    const holder=select.parentElement;
    const combo=holder?.querySelector?.(".aux-cor-combo");
    if(!(combo instanceof HTMLElement))return false;
    const label=combo.querySelector(".aux-cor-btn .aux-cor-label");
    if(label instanceof HTMLElement){
      const opt=select.selectedOptions&&select.selectedOptions[0]?select.selectedOptions[0]:null;
      label.textContent=String(opt?.textContent||"");
    }
    combo.querySelectorAll(".aux-cor-item").forEach((item,idx)=>item.classList.toggle("active",idx===select.selectedIndex));
    return true;
  }

  function toolbarSincronizarToolbarFormato(cfg,deps={},force=false){
    if(!cfg?.page||!cfg?.panel)return false;
    if(cfg.panel.classList.contains("hidden"))return false;
    const selecaoDentroPagina=typeof deps.selecaoDentroPagina==="function"?deps.selecaoDentroPagina:null;
    const dentro=selecaoDentroPagina?!!selecaoDentroPagina():false;
    const ativoNoEditor=document.activeElement===cfg.page||cfg.page.contains(document.activeElement);
    if(!force&&!dentro&&!ativoNoEditor)return false;
    const sincronizarEstruturaParagrafoAtual=typeof deps.sincronizarEstruturaParagrafoAtual==="function"?deps.sincronizarEstruturaParagrafoAtual:null;
    if(sincronizarEstruturaParagrafoAtual)sincronizarEstruturaParagrafoAtual();
    const obterElementoSelecao=typeof deps.obterElementoSelecao==="function"?deps.obterElementoSelecao:null;
    const focoEl=obterElementoSelecao?obterElementoSelecao():null;
    const foco=focoEl||cfg.page;
    const estilo=foco?window.getComputedStyle(foco):null;
    const queryCommandState=typeof deps.queryCommandState==="function"?deps.queryCommandState:null;
    const queryCommandValue=typeof deps.queryCommandValue==="function"?deps.queryCommandValue:null;
    const normalizarFonteNome=typeof deps.normalizarFonteNome==="function"?deps.normalizarFonteNome:(valor=>String(valor??"").split(",")[0].replace(/["']/g,"").trim());
    const resolverTamanhoFonteUi=typeof deps.resolverTamanhoFonteUi==="function"?deps.resolverTamanhoFonteUi:(valor=>String(valor??"").trim());
    const corParaHex=typeof deps.corParaHex==="function"?deps.corParaHex:(valor=>String(valor??"").trim());
    let bold=queryCommandState?!!queryCommandState("bold"):false;
    let italic=queryCommandState?!!queryCommandState("italic"):false;
    let underline=queryCommandState?!!queryCommandState("underline"):false;
    if(estilo){
      if(!bold){
        const fwRaw=String(estilo.fontWeight||"").trim().toLowerCase();
        const fwNum=Number.parseInt(fwRaw,10);
        bold=(Number.isFinite(fwNum)&&fwNum>=600)||fwRaw==="bold";
      }
      if(!italic){
        const fs=String(estilo.fontStyle||"").trim().toLowerCase();
        italic=fs.includes("italic")||fs.includes("oblique");
      }
      if(!underline){
        underline=String(estilo.textDecorationLine||estilo.textDecoration||"").toLowerCase().includes("underline");
      }
    }
    let alinhamento="left";
    if(queryCommandState&&queryCommandState("justifyCenter"))alinhamento="center";
    else if(queryCommandState&&queryCommandState("justifyRight"))alinhamento="right";
    else if(queryCommandState&&queryCommandState("justifyFull"))alinhamento="justify";
    else{
      let textAlign=String(estilo?.textAlign||"").toLowerCase();
      if(!textAlign&&focoEl?.closest){
        const block=focoEl.closest("p,div,li,td,th,section,article,h1,h2,h3,h4,h5,h6");
        if(block)textAlign=String(window.getComputedStyle(block).textAlign||"").toLowerCase();
      }
      if(textAlign.includes("center"))alinhamento="center";
      else if(textAlign.includes("right")||textAlign.includes("end"))alinhamento="right";
      else if(textAlign.includes("justify"))alinhamento="justify";
      else alinhamento="left";
    }
    toolbarSetButtonAtivo(cfg.btnNegrito,bold);
    toolbarSetButtonAtivo(cfg.btnItalico,italic);
    toolbarSetButtonAtivo(cfg.btnSublinhado,underline);
    toolbarSetButtonAtivo(cfg.btnEsq,alinhamento==="left");
    toolbarSetButtonAtivo(cfg.btnCentro,alinhamento==="center");
    toolbarSetButtonAtivo(cfg.btnDir,alinhamento==="right");
    toolbarSetButtonAtivo(cfg.btnJustificar,alinhamento==="justify");
    const fontAtual=(()=>{
      const viaCmd=normalizarFonteNome(queryCommandValue?queryCommandValue("fontName"):"");
      if(viaCmd)return viaCmd;
      const viaCss=normalizarFonteNome(estilo?.fontFamily||"");
      return viaCss||"Arial";
    })();
    if(cfg.font instanceof HTMLSelectElement){
      const opts=[...cfg.font.options];
      const alvo=fontAtual.toLowerCase();
      const match=opts.find(opt=>String(opt.value||"").trim().toLowerCase()===alvo)||opts.find(opt=>String(opt.textContent||"").trim().toLowerCase()===alvo);
      if(match&&cfg.font.value!==match.value)cfg.font.value=match.value;
    }
    const tamanhoAtual=(()=>{
      const viaCmd=resolverTamanhoFonteUi(queryCommandValue?queryCommandValue("fontSize"):"");
      if(viaCmd)return viaCmd;
      const viaCss=resolverTamanhoFonteUi(estilo?.fontSize||"");
      return viaCss||"11";
    })();
    if(cfg.size instanceof HTMLSelectElement){
      const has=[...cfg.size.options].some(opt=>String(opt.value||"")===tamanhoAtual);
      if(has&&cfg.size.value!==tamanhoAtual)cfg.size.value=tamanhoAtual;
    }
    const corAtual=(()=>{
      const viaCmd=corParaHex(queryCommandValue?queryCommandValue("foreColor"):"");
      if(viaCmd)return viaCmd;
      return corParaHex(estilo?.color||"")||"#000000";
    })();
    if(cfg.color instanceof HTMLSelectElement){
      const opts=[...cfg.color.options];
      const match=opts.find(opt=>String(opt.value||"").trim().toLowerCase()===corAtual);
      if(match&&cfg.color.value!==match.value){
        cfg.color.value=match.value;
        toolbarAtualizarVisualComboCor(cfg);
      }
    }
    if(cfg.colorSwatch instanceof HTMLElement){
      cfg.colorSwatch.style.backgroundColor=corAtual||"#000000";
    }
    return true;
  }

  function toolbarAgendarSincronizarToolbar(cfg,deps={},force=false){
    if(!cfg)return false;
    if(force){
      cfg.toolbarSyncScheduled=false;
      return toolbarSincronizarToolbarFormato(cfg,deps,true);
    }
    if(cfg.toolbarSyncScheduled)return false;
    cfg.toolbarSyncScheduled=true;
    const flush=()=>{
      if(!cfg)return;
      cfg.toolbarSyncScheduled=false;
      toolbarSincronizarToolbarFormato(cfg,deps,false);
    };
  ...
  const toolbarApi=Object.freeze({
    atualizarVisualComboCor:toolbarAtualizarVisualComboCor,
    agendarSincronizarToolbar:toolbarAgendarSincronizarToolbar,
    setToolbarButtonAtivo:toolbarSetButtonAtivo,
