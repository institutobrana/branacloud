...
def aplicar_compatibilidade_schema() -> None:
    ...
    with engine.begin() as conn:
        ...
        conn.execute(text("ALTER TABLE usuarios ADD COLUMN IF NOT EXISTS last_seen_at TIMESTAMP WITH TIME ZONE"))
        ...
        conn.execute(text("ALTER TABLE IF EXISTS anamnese_perguntas ALTER COLUMN tipo_pergunta SET DEFAULT 1"))
        conn.execute(text("ALTER TABLE IF EXISTS anamnese_perguntas ALTER COLUMN tipo_resposta SET DEFAULT 1"))
        conn.execute(text("ALTER TABLE IF EXISTS anamnese_perguntas ALTER COLUMN tipo_pergunta SET NOT NULL"))
        conn.execute(text("ALTER TABLE IF EXISTS anamnese_perguntas ALTER COLUMN tipo_resposta SET NOT NULL"))
        conn.execute(
            text(
                """
