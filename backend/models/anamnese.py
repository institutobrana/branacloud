...
class AnamnesePergunta(Base):
    ...
    tipo_pergunta = Column(Integer, nullable=False, default=1)
    tipo_resposta = Column(Integer, nullable=False, default=1)
    texto = Column(String(400), nullable=False)
    mensagem_alerta = Column(String(255), nullable=True)
    ativo = Column(Boolean, nullable=False, default=True)
    criado_em = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    atualizado_em = Column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now(), nullable=False)
