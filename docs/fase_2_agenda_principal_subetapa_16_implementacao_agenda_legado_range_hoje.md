# Fase 2 - Agenda principal - Subetapa 16 - Implementacao minima do helper puro agendaLegadoRangeHoje e validacao manual do impacto visual nos filtros de periodo da agenda legado

## 1. Objetivo
Extrair minimamente o helper puro `agendaLegadoRangeHoje` de `frontend/app.js` para o modulo ja existente de utilitarios da agenda legado, preservando exatamente a semantica atual e sem alterar o comportamento do botao/filtro Hoje.

## 2. Escopo
Escopo desta subetapa:

- mover somente `agendaLegadoRangeHoje` para o modulo ja criado;
- manter os pontos de uso existentes;
- preservar a semantica atual do range de hoje;
...
| `agendaLegadoRangeHoje` | `frontend/app.js:8903` | Montar faixa de hoje | data atual | `{inicio, fim}` | Parcial, temporal | Nao | Sim, tempo atual | Nao | Nao | Sim | Nao | Sim, afeta filtros de periodo | Baixo | Sim, clicando botao Hoje | Sim, cabe no modulo atual | **Sim, candidato recomendado** |
| `agendaLegadoRangeSemana` | `frontend/app.js:8908` | Montar faixa de 7 dias | data atual | `{inicio, fim}` | Parcial, temporal | Nao | Sim, tempo atual | Nao | Nao | Sim | Nao | Sim, afeta filtros de periodo | Baixo a medio | Sim, clicando botao Semana | Sim, cabe no modulo atual | Nao como quinto recorte |
| `agendaLegadoParseDataInput` | `frontend/app.js:8379` | Converter input de data para ISO | `valor` | `aaaa-mm-dd` ou vazio | Sim, mas toca regra de saida do payload | Nao | Nao | Nao | Nao | Nao | Nao | Sim, afeta salvar/editar | Medio | Sim, conferindo salvar/editar | Sim, cabe no modulo atual | Nao como quinto recorte |
| `agendaLegadoCoerceHoraTexto` | `frontend/app.js:8520` | Normalizar horas de input/blur | `value`, flags de formato | `HH:MM` ou vazio | Sim, mas mais complexa | Nao | Nao | Nao | Nao | Nao | Nao | Sim, afeta campo de hora | Medio | Sim, mas com mais cenario | Sim, cabe no modulo atual | Nao como quinto recorte |
| `agendaLegadoNormalizarHexCor` | `frontend/app.js:9005` | Normalizar cor hex | `value` | `#rrggbb` ou vazio | Parcial, possui fallback a helper global | Nao | Nao | Nao | Nao | Nao | Nao | Sim, afeta cor/visual | Medio | Sim, conferindo cor renderizada | Sim, cabe no modulo atual | Nao como quinto recorte |

## 8. Candidato recomendado para quinta implementacao futura
Candidato recomendado:

- `agendaLegadoRangeHoje`

## 9. Justificativa da escolha
A escolha de `agendaLegadoRangeHoje` e a mais equilibrada para o quinto recorte porque:
...
- o impacto visual e previsivel e limitado aos campos de periodo do filtro da agenda legado;
- cabe naturalmente no modulo ja criado `frontend/js/modules/agenda-principal-legado-utils.js`;
- evita os helpers de maior risco ligados a querystring, payload ou interacao de hora;
- e mais simples de testar manualmente do que os helpers de range semanal ou normalizacao de cor.

Comparacao resumida:
- os helpers `agendaSemana*` dependem diretamente de URL/querystring e navegacao, entao nao sao o quinto recorte mais seguro;
- `agendaLegadoRangeSemana` e temporalmente mais amplo que `agendaLegadoRangeHoje` e altera dois limites de periodo, elevando o risco;
...
- `agendaLegadoRangeHoje` permanece o menor recorte util restante com impacto restrito a um unico intervalo de periodo.
...
- `agendaLegado.btnHoje.addEventListener("click", ...)` continua chamando `agendaLegadoRangeHoje()`;
- o resultado continua alimentando `agendaLegado.inputInicio.value` e `agendaLegado.inputFim.value`;
- a chamada posterior a `agendaLegadoCarregar()` foi mantida;
- nao houve alteracao no fluxo do botao/filtro Hoje.
...
- `inicio` e `fim` continuam recebendo a mesma data ISO no formato `aaaa-mm-dd`;
- o comportamento continua a mesma expectativa de faixa de hoje na agenda legado.
...
- a data usada e a data local corrente do navegador no instante da execucao.

## 16. Confirmacao sobre formato retornado
Confirmacao sobre o formato retornado:

- o retorno continua sendo um objeto com as chaves `inicio` e `fim`;
- ambos os campos continuam em formato ISO `aaaa-mm-dd`;
- o tipo do retorno continua sendo um objeto simples.

## 17. Confirmacao sobre inicio/fim do range
Confirmacao sobre inicio/fim:

- `inicio` continua igual ao dia corrente;
- `fim` continua igual ao dia corrente;
- nao houve ampliacao para faixa semanal ou qualquer outra regra.

## 18. Confirmacao de que o botao/filtro Hoje nao teve regra alterada
Confirmacao sobre o botao/filtro Hoje:

- o botao/filtro Hoje continua acionando a mesma logica funcional;
- a unica alteracao foi a extracao do helper para o modulo ja existente;
- nao houve mudanca na regra de negocio do filtro.
...
## 21. Fronteira exata da futura extracao
Fronteira futura pretendida:

- extrair apenas a montagem da faixa de hoje para os filtros da agenda legado;
- manter o comportamento atual:
  - criar `new Date()` no momento da chamada;
  - usar `toISOString().slice(0,10)` para inicio e fim;
  - retornar o mesmo objeto `{ inicio, fim }` com a mesma data em ambos os campos.
...
## 22. Se o helper deve entrar no modulo ja criado ou exigir novo modulo
...
## 23. O que NAO deve entrar na futura extracao
...
- qualquer ajuste de visual fora do periodo de hoje;
- qualquer alteracao de payload.

## 24. Riscos tecnicos
Riscos tecnicos da futura extracao:

- alterar a faixa de periodo usada pelo botao Hoje;
- quebrar a expectativa de inicio e fim iguais na data corrente;
...
## 25. Validacao tecnica executada
Validacao tecnica executada nesta etapa:

- `node --check frontend/app.js`
- `node --check frontend/js/modules/agenda-principal-legado-utils.js`

## 26. Plano de teste manual para o usuario
Quando houver validacao manual futura, testar:

1. Abrir `Agenda principal` e entrar na `agenda-legado`.
2. Acionar o botao de faixa de hoje.
3. Conferir se os campos de inicio e fim continuam iguais para a data corrente.
4. Verificar se a listagem continua carregando sem erro.
5. Trocar a data do sistema, se necessario, e confirmar que a faixa continua coerente com o dia corrente.
6. Conferir console sem `ReferenceError` ou `TypeError`.

## 27. Itens explicitamente fora do escopo
...
## 28. Proxima subetapa recomendada
Proxima subetapa recomendada:

- `Agenda principal - Subetapa 17 - Validacao manual da extracao de agendaLegadoRangeHoje e revisao do impacto visual nos filtros de periodo da agenda legado`
...
## 30. Registro para roadmap
- A Subetapa 16 foi concluida como implementacao minima do quinto helper puro.
- O quinto helper extraido foi `agendaLegadoRangeHoje`.
...
- O proximo passo recomendado e a validacao manual da extracao de `agendaLegadoRangeHoje` com impacto visual nos filtros de periodo da agenda legado.

## 31. Commit seletivo obrigatorio
