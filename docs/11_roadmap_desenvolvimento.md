...
- **INTEGRAÇÃO DE PACIENTES:** `IMPLEMENTED` com APIs versionadas de Pacientes (`listarPacientes`/`obterPaciente`), sem transporte da arquitetura dirty `features/menuPacientes`.
...
- **GOOGLE ID TOKEN REAL VALIDATION:** `BLOCKED`; `ROOT LOCATION = ID_TOKEN_VALIDATION`.
- **GOOGLE OAUTH CONNECTION:** `BLOCKED_AT_ID_TOKEN_VALIDATION`.
...
- **CLASSIFICAÇÃO DO FREEZE:** código e contratos Agenda estão `IMPLEMENTED`; evidências estáticas estão `TESTED`; o runtime local existente está `RUNTIME_VALIDATED`; a validação real do id_token e a escrita/exportação Google permanecem `BLOCKED` e `NOT_YET_RELEASED`.
