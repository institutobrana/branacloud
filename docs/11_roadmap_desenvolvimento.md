...
- Auditoria runtime posterior confirmou que o host da tela principal odontologica nao estava sendo carregado no `frontend/index.html`; a integracao dos modulos `tela-principal-odontologica-*` foi adicionada ao carregamento da pagina, a tela principal passou a montar no workspace real e o fluxo `Tratamento -> Novo tratamento` deixou de cair em `Ficha pessoal` nesse caso especial.
...
- A proxima etapa apos a correcao runtime do header e confirmar visualmente no navegador a exibição sem paciente e com paciente ativo, antes de voltar ao fluxo `Menu de pacientes -> Ficha pessoal -> Novo tratamento`.
