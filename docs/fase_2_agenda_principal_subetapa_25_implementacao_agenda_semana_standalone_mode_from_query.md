# Fase 2 - Agenda principal - Subetapa 25 - Implementacao minima do helper puro agendaSemanaStandaloneModeFromQuery e validacao manual do modo standalone da agenda semana

## 1. Objetivo
Extrair minimamente o helper puro `agendaSemanaStandaloneModeFromQuery` de `frontend/app.js` para o modulo proprio da `Agenda semana`, preservando exatamente a semantica atual de leitura do modo standalone e sem alterar o comportamento da navegacao.

## 2. Escopo
Escopo desta subetapa:

- mover somente `agendaSemanaStandaloneModeFromQuery` para o modulo proprio da agenda semana;
...
Na descricao anterior da Subetapa 23, a validacao manual direta da querystring standalone tinha ficado limitada porque o usuario ainda nao sabia a rota exata. Essa limitacao foi superada depois, e o resultado foi positivo.
...
- e o proximo passo natural apos `agendaSemanaIsStandaloneRequest`, completando a fronteira de leitura da querystring do modo standalone;
...
- o usuario ja conseguiu validar posteriormente a URL/standalone, entao ha uma base pratica para consolidar a fronteira.
...
- `Agenda principal - Subetapa 26 - Validacao manual da extracao de agendaSemanaStandaloneModeFromQuery e revisao do modo standalone da agenda semana`
...
- A Subetapa 25 foi concluida como implementacao minima do oitavo helper puro.
...
- `frontend/index.html` nao precisou ser alterado, porque o modulo ja estava carregado antes de `frontend/app.js`.
...
- O proximo passo recomendado e a validacao manual da extracao de `agendaSemanaStandaloneModeFromQuery` e do modo standalone da agenda semana.
