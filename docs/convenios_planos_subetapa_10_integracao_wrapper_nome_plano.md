# Convênios e Planos — Subetapa 10 — Integração funcional mínima do wrapper de nome de plano

## 1. Objetivo da etapa
Registrar a verificação da integração funcional mínima do wrapper `convPlanNormalizarNomePlanoLocal(valor)` em `frontend/app.js`, preservando o fallback local e sem alterar payload, validação, salvamento, exclusão, eventos, renderização, modais ou vínculo funcional.
...
## 6. Caminho real do helper `normalizarNomePlano`
...
## 7. Estado anterior do wrapper `convPlanNormalizarNomePlanoLocal(valor)`
...
```js
function convPlanNormalizarNomePlanoLocal(valor)
```
...
## 20. Se dependência de convênio selecionado foi preservada
Sim. A dependência indireta do fluxo de plano em relação ao convênio selecionado foi preservada porque não houve mudança no wrapper nem em camadas superiores.

## 21. Se pacientes, procedimentos, tabelas, preços, custos, reajustes e financeiro foram preservados
Sim. Não houve alteração em pacientes, procedimentos, tabelas, preços, custos, reajustes ou financeiro.

## 22. Se backend/banco/schema/migrations/endpoints foram preservados
Sim. Não houve alteração em backend, banco, schema, migrations ou endpoints.

## 23. Resultado dos checks
...
## 24. Riscos residuais
- O wrapper já está defensivo, mas qualquer mudança futura no contrato de `window.BranaConveniosPlanosModule.helpers.normalizarNomePlano` ainda pode afetar a cadeia de normalização.
...
6. Selecionar um plano existente, sem salvar.
7. Abrir a inclusão/alteração de plano, se existir botão visual para isso.
8. Conferir ou digitar o campo de nome do plano, sem salvar.
9. Fechar/cancelar o modal.
10. Confirmar que listas continuam aparecendo.
11. Confirmar que clique simples/seleção visual continua funcionando.
12. Verificar o console do navegador.
...
## 26. Decisão recomendada para próxima etapa
